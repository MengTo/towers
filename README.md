...
`index.html` is 3.1 MB, and most of that is payload rather than code: a vendored Three.js r149 build (593 KB), the texture set (907 KB) and the audio (1.5 MB), all inline. Nothing is fetched at runtime, so that figure is the whole download and there is never a second request.
